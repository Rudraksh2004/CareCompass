...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-20">
      ...
      <div className="relative group overflow-hidden rounded-[2.5rem] border border-white/80 dark:border-white/[0.05] bg-white/[0.4] dark:bg-[#030712]/40 backdrop-blur-[60px] p-10 transition-all duration-700 hover:shadow-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Emergency Node */}
        {emergencyProfile && (
          <div className="relative group overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-red-600 to-rose-700 p-10 flex flex-col justify-between h-[300px]">
             <div className="relative z-10">
                <div className="flex items-center gap-4 mb-4">
                   <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-red-600 shadow-xl">
                      <AlertCircle size={24} />
                   </div>
                   <h3 className="text-3xl font-black text-white tracking-tighter uppercase">Emergency Protocol</h3>
                </div>
                <p className="text-red-100/60 font-bold text-sm tracking-widest">Medical ID: {user?.uid ? user.uid.slice(0, 10).toUpperCase() : "AUTH-REF"}</p>
             </div>
             <div className="relative z-10 flex items-center justify-between">
                <div className="flex gap-4">
                   <span className="px-6 py-3 rounded-xl bg-white/10 text-white text-xs font-black uppercase tracking-widest border border-white/20">Blood: {emergencyProfile.bloodGroup || "-"}</span>
                </div>
                <Link href="/dashboard/emergency" className="px-8 py-4 rounded-2xl bg-white text-red-600 font-black text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-transform">
                   Access
                </Link>
             </div>
          </div>
        ...
        {/* AI Predictor Scan */}
        <div className="relative group overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-indigo-600 to-blue-700 p-10 flex flex-col justify-between h-[300px]">
           <div className="relative z-10">
              <div className="flex items-center gap-4 mb-4">
                 <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-indigo-600 shadow-xl">
                    <Activity size={24} />
                 </div>
                 <h3 className="text-3xl font-black text-white tracking-tighter uppercase">Disease Predictor</h3>
              </div>
              <p className="text-indigo-100/60 font-bold text-sm tracking-widest leading-relaxed max-w-sm">Execute hybrid bio-marker analysis and regional epidemiological synthesis.</p>
           </div>
           <div className="relative z-10 text-right">
              <Link href="/dashboard/disease-predictor" className="inline-block px-10 py-4 rounded-2xl bg-white text-indigo-600 font-black text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-transform">
                 Initiate Bio-Sync →
              </Link>
           </div>
        </div>
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { icon: <FileText size={20} />, title: "Report Explainer", desc: "Neuro-linguistic analysis.", route: "/dashboard/report", color: "blue" },
            { icon: <Stethoscope size={20} />, title: "Predictor Node", desc: "Diagnostic synthesis engine.", route: "/dashboard/disease-predictor", color: "indigo" },
            { icon: <Pill size={20} />, title: "Prescription Sync", desc: "Pharmacological digitizer.", route: "/dashboard/prescription", color: "emerald" },
            { icon: <MessageSquare size={20} />, title: "Virtual Consultation", desc: "High-fidelity AI consultation.", route: "/dashboard/chat", color: "indigo" },
            { icon: <Clock size={20} />, title: "Medicine Tracker", desc: "Pulse scheduling & relay.", route: "/dashboard/reminders", color: "rose" },
            { icon: <Microscope size={20} />, title: "Bio-Catalog", desc: "Deep pharmaceutical ledger.", route: "/dashboard/medicine", color: "amber" }
          ].map((feature, i) => (
            <Link key={i} href={feature.route} className="group relative overflow-hidden rounded-[2rem] border border-white/80 dark:border-white/[0.05] bg-white/[0.4] dark:bg-[#030712]/40 backdrop-blur-md p-8 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl">
               ...
               <div className="flex items-center gap-5 relative z-10">
                  <div className={`w-12 h-12 rounded-xl bg-${feature.color}-500/10 flex items-center justify-center text-${feature.color}-600 dark:text-${feature.color}-400`}>
                    {feature.icon}
                  </div>
                  <div>
